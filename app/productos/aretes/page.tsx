...
export default function PLParetes() {
  ...
  useEffect(() => {
    fetch("/productos/allProducts.json")
      ...
      .then((data) => {
        const aretesCategory = data.categorias.find(
          (category: Category) => category.nombre === "Aretes"
        );
        if (aretesCategory) {
          setProducts(aretesCategory.productos);
          setFilteredProducts(aretesCategory.productos);
        }
      })
    ...
    <Center py={12} flexDirection="column" alignItems="center">
      ...
      <Box mb={5}>
        <Button
          ...
          borderRadius={80}
          color="#ffffff"
          _hover={{ transform: "scale(1.05)", boxShadow: "lg", bg: "#e54f74" }}
        >
          Mostrar Todo
        </Button>
        <Button
